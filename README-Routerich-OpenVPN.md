...
2. Перейдите: **VPN** → **OpenVPN**

   ![Страница OpenVPN — загрузка конфига](images/openvpn_upload_config.png)

3. В разделе **«Загрузка конфигурационного файла OVPN»** в поле **«Instance name»** напишите любое удобное имя на латинице, например `OVPN`
4. Нажмите **«Выберите файл»**, выберите скачанный `.ovpn` и нажмите **Загрузить**
5. Файл появится в списке конфигураций сверху
...
2. Заполните параметры:
   - **Имя**: `OVPN` (или любое латиницей, например `antizapret`)
   - **Протокол**: `Неуправляемый`
   - **Устройство**: в выпадающем списке выберите самый низ — **«Пользовательский»**, и впишите вручную `tun0` (интерфейса ещё нет, т.к. OpenVPN пока не запущен)

   ![Создание нового интерфейса — tun0 вручную](images/openvpn_new_interface.png)
...
3. Настройте следующие параметры:
   ...
   ![Настройка зоны фаервола anti_fw](images/openvpn_firewall_zone.png)

   - **Входящий трафик**: `принимать`
   - **Выход**: `принимать`
   - **Внутризональная пересылка**: `принимать`
   - ✅ **Маскарадинг** — включено
   - ✅ **Ограничение MSS** — включено
   - **Охватываемые сети**: выберите созданный интерфейс OpenVPN (например `TU`)
